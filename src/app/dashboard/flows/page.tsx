            ...
            <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
              {flows.map((flow) => {
                ...
                return (
                  <div
                    key={flow.id}
                    className="flex flex-col justify-between rounded-2xl border border-border bg-card p-5 hover:border-[#fe5105]/50 transition-colors shadow-sm"
                  >
                    <div>
                      ...
                      <h3 className="text-sm font-bold text-foreground">{flow.name}</h3>
                      <p className="mt-1 text-xs text-muted-foreground line-clamp-2">
                        {flow.categories?.[0] || 'Vente & Devis WhatsApp'} • {screenCount} Écran{screenCount > 1 ? 's' : ''}
                      </p>
                    </div>
                    ...
                    <div className="mt-5 flex flex-wrap items-center justify-end gap-2 border-t border-border pt-4">
                      <button
                        ...
                        className="rounded-full bg-secondary px-3 py-1.5 text-[11px] font-semibold text-foreground hover:bg-secondary/80 transition-colors"
                      >
                        Tester
                      </button>
                      <button
                        onClick={() => openEditFlowBuilder(flow)}
                        className="rounded-full bg-[#fe5105] px-3 py-1.5 text-[11px] font-bold text-white hover:bg-[#e04602] transition-colors"
                      >
                        Modifier
                      </button>
                      <button
                        onClick={() => handleDeleteFlow(flow)}
                        className="rounded-full bg-destructive/10 px-3 py-1.5 text-[11px] font-semibold text-destructive hover:bg-destructive/20 transition-colors"
                      >
                        Supprimer
                      </button>
                    </div>
